...
def call_post_event(description, image, channel_name, event_name, meeting_link, server_name, event_date,event_time, event_duration=1):
    @client.event
    async def on_ready():
        print(f"Logged in as {client.user}")
        await post_event(description, image, channel_name, event_name, meeting_link, server_name, event_date,event_time, event_duration)
        await client.close()
...
async def post_event(description, image, channel_name, event_name, meeting_link, server_name, event_date,event_time, event_duration=1):
    ...
    try:
        ...
        event_start_time = datetime.strptime(f"{event_date} {event_time}", "%Y-%m-%d %H:%M").replace(tzinfo=timezone.utc)
